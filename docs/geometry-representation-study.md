...
> **상태: ✅ 완료 (자율 야간 작업, 2026-06-15) + §8 라운드트립 정밀화 추가 (세션 Q&A, 2026-06-17).** 3 딥리서치 패스(1a 패러다임·1b 커널·2 협업/Figcad-hard) + 코드 그라운딩 + F-rep 직접 fetch 검증. 중단조건 5개 충족.
...
- **경로 B — 불변① 진화(freeform 네이티브)**: 필요시. **단 OCCT B-rep 커널 아니라 F-rep/SDF 레인 권장**(§7 — 더 가볍고 persistent-naming 없고 머지가능, wasm32 4GB 천장 회피). off-identity(Figcad는 Rhino 대체 아님)라 hard 요구 전엔 보류.
...
**결론**: "만능 단일 표현으로 모든 툴을 네이티브 수용"은 환상(커널 물리). 현실 최선 = **Lane-1 공유 파라미터로 의미요소 네이티브-everywhere + Lane-2 네이티브 recipe 보관으로 출신툴 무손실**. Figcad 현 모델이 이미 이 길 위에 있고, 빠진 것 = (협업) 머지 lint(§6 1단계) · **Lane-2를 "저장 참조"로 확장**(현재는 메시 표시만 — 출신툴 무손실 왕복하려면 원본 페이로드 저장·동기화, 별도 표현으로) · (나중) freeform F-rep/SDF 레인(§7) · (v1.5) AI 리프팅으로 Lane-2→Lane-1 승격. = §0~§7의 직접 귀결, 새 기하 커널 아님.
