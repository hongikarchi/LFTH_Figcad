...
| **.3dm** | `3dm` | 3D 메시 오버레이 | `rhino3dm` WASM, **Mesh 객체만** | 🟡 기존. Brep/블록-only 파일 = 빈 결과(명확 안내 → glTF export 권장). |
...
- **.3dm = Mesh-only**: rhino3dm은 브라우저서 Brep 테셀레이션 불가(풀 커널 필요). Brep 모델 = 빈 오버레이 + 안내. → Rhino서 glTF/메시 export, 또는 Rhino 커넥터(`FigcadPushBreps`).
