...
- **Primary = Railway (라이브 배포중)**: **https://lfthfigcad-production.up.railway.app** (Node 백엔드, Dockerfile). 최신 배포 = **레벨 구조화 M1~M4 (서버 M2) (2026-07-16)**(라이브 `index-CoR15s0e.js`, master `8482c86`, deployment `ac042c43`). 서버가 dedup 절대z 정규화·update_element 층이동·band lint 배포본 = **커넥터 v0.7(층 자동 구조화 토글)이 이제 안전하게 왕복 가능**(구서버 전요소 스킵 회피). CF(`figcad.archivibe.workers.dev`)는 rollback용. 배포가이드 `docs/RAILWAY_DEPLOY.md`. 배포 머신 = bluem 데스크톱(railway CLI 로그인됨, `railway up --detach`).
...
- **레벨 구조화 M1~M4 구현 완료 (2026-07-13, 미배포 — `docs/level-structuring-plan.md`)**: G2 잔여 멀티레벨 배정.
  - **M1+M2 (TS, `0caeaa0`)**: connectorDedup 절대z 정규화(평탄 푸시↔층 구조화 재푸시 크로스레벨 매칭·v1 back-compat) · apply.ts dedup 프리패스 delete 해제+add_level 혼합 400 가드 · update_element baseOffset/levelId(실증 버그)+층이동 가드 · lint level-band-mismatch(info, 평탄 푸시 감지). adversarial 리뷰 제기 13/확정 5 전부 수정.
  - **M2 감지기+M3 방출 (C#)**: `FigcadStories.cs`(순수, 갭분할·면적가중 중앙값·지붕강등, dotnet test 14/14) · `FigcadConnector` multiLevel(POST-A add_level 재사용매치·토큰 치환 前 dedup·per-item 레벨 배정, OFF=v0.6 동일) · 패널 토글 OFF 기본 · TestHarness 골든 다층(GoldenMultiPush·StoryCensusToFile — Rhino MCP 실행 deferred).
  - **M4**: story-smoke.mjs(커넥터 프로토콜 헤드리스 — miniflare) · README v0.7 · 이 갱신.
  - **리뷰 완료 (`b9c762b`)**: S1(TS) 제기 13/확정 5 + S2/S3(C#) 제기 11/확정 7(opus 36에이전트) 전부 수정. 주요: 지붕 강등 오탐(슬라브만 남은 실층 붕괴 차단)·병합 캐스케이드·census≠emission 앵커셋 통일·add_level order 충돌·구서버 전요소 스킵.
  - **검증 (전 그린)**: typecheck ✅ · vitest 688(core 462·server 46·interop 109·web 71) ✅ · dotnet test 16/16 ✅ · story-smoke ✅ · 스모크 28/29(comment-e2e=환경 flaky, 레벨 트랙 무관). **남은 것 = 사용자 Rhino 세션만**: GoldenMultiPush ×2 · StoryCensusToFile 260629 · 배포(서버 M2 먼저).
