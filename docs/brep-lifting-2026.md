...
## 부록 v0.5 확정 임계값 (2026-07-02 — 골든 씬 + 실파일 캘리브레이션)

검증 하네스 = `connectors/rhino/testlib/`(헤드리스 골든 씬 14객체, Rhino MCP로 DLL 로드) + `apps/web/scripts/connector-golden.mjs`(18단언·2회 push dedup). 실파일 회귀망 = 260629 main ms.3dm(5867객체) census-only.

| 게이트 | 값 | 근거 |
|---|---|---|
| aspect(보 축길이 > 단면최대변 + ε) | ε = **1mm** | 종전 정확비교는 800³ 큐브서 float 동점 코인플립(실증). 비율 ×1.2는 실존 인방/커플링보(900×800)까지 기각 → 과잉으로 철회 |
| 보 단면 폭(수평) 상한 | **1200mm** | 평판 2000×150 → 보 오분류 차단(골든 실증). 폭 2m 보는 없다 |
| 보 단면 춤(수직) 상한 | **2500mm** | 전이거더 1500~2500 실존 — max(폭,춤) 단일상한 1200은 전이거더 기각(리뷰 지적) → 축별 분리 |
| 기둥 단면 최대변 상한 | **2000mm** | 메가기둥 1300~1800 실존 여유 |
| 벽 두께 상한 | **1500mm** | 600은 실파일 F-Wall 600~700 파사드 벽 5개 기각(census 실증) → 지하 옹벽 여유까지 1500 |
| 상한 적용 조건 | 레이어 **자동분류에만** | `figcad:kind`/레이어맵 명시 지정 = 사용자 의사 → 상한 스킵(기하 유효성 게이트는 유지) |
| 계단 tread 검출 | 상향 수평면 z-클러스터 ≥3 · riser 50~400mm · 편차 ≤max(5mm,20%) · 선형 진행(역행/폭초과 이탈 기각) | 실파일 계단 20/32 파라 리프트(riser 150/167 실측), 곡선 12 = bbox 폴백 |
| 게이트 측정 지점 | 핏 **이전** raw pts bbox (기둥·보 동일) | 사후 명명치수 게이트는 필렛 sharpen 편차로 kind별 불일치 + fail-open(리뷰 지적) |

미해결 잔여: 폴리곤 면적 보존 게이트가 홀수 치수 반올림에 기각하는 케이스 실파일 3건(설계된 정직 거동, 관찰만) · L-PARKING = 매칭 kind 없음 · 곡선 계단/난간 실기하.
