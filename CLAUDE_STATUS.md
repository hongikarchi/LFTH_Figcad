...
- **P5 Railway 설정**: Dockerfile·railway.json·docs/RAILWAY_DEPLOY.md.

## 검증 게이트 (전부 green)
core 353·interop 41(+3)·server 13(+3)·tsc 0·web build·node 번들. 멀티플레이어/origin/fed/version/AI배선/영속 로컬 작동.

## 다음 (사용자)
- **AI 키**: `railway variables --set ANTHROPIC_API_KEY=...` + Railway 리전 US 확인.
- **커넥터 BASE**: `https://lfthfigcad-production.up.railway.app`로 갱신. CF 워커는 롤백용으로만 유지.
- **오버레이 fix(M14.1)** 포함됨(web 코드). 이전 미배포분(M12/M13/M13.5/M13.6/M14.1)은 M15 Railway 빌드에 포함 완료.
