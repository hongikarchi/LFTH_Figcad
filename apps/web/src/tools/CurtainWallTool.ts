...
/**
 * 커튼월 그리기 — 베이스라인 a→b 클릭 체인(BeamTool 패턴). 멀리언 그리드는
 * uSpacing/vSpacing 기본값으로 생성, InfoBox에서 조정. 단면=활성 커튼월 타입.
 */
export class CurtainWallTool implements Tool {
  ...
  constructor(private ctx: EditorContext) {
    this.ghostMesh = new THREE.Mesh(
      ...
      new THREE.MeshLambertMaterial({ color: 0x8fa8b8, transparent: true, opacity: 0.5 }),
    );
    this.ghostEdges = new THREE.LineSegments(
      new THREE.BufferGeometry(),
      new THREE.LineBasicMaterial({ color: 0x5a7a8c }),
    );
  ...
  private commit(end: Pt): void {
    ...
    if (Math.hypot(end[0] - start[0], end[1] - start[1]) >= 50) {
      this.ctx.store.createCurtainWall({
        levelId: this.ctx.levelId(),
        typeId: this.ctx.typeId('curtainwall'),
        a: start,
        b: end,
        uSpacing: DEFAULT_SPACING,
  ...
  private updateGhost(a: Pt, b: Pt): void {
    const type = this.ctx.store.getType(this.ctx.typeId('curtainwall')) as CurtainWallType | undefined;
    const level = this.ctx.store.getLevel(this.ctx.levelId());
    if (type?.kind !== 'curtainwall' || !level) return;
    const lenMm = Math.hypot(b[0] - a[0], b[1] - a[1]);
    if (lenMm < 1) {
      this.hideGhost();
      return;
    }
    const ghost: CurtainWallElement = {
      id: '__ghost__',
      kind: 'curtainwall',
      levelId: level.id,
      typeId: type.id,
    ...
    const geo = deriveCurtainWall({ cw: ghost, type, level });
